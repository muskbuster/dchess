...
export class WithdrawAll extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type WithdrawAll must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("WithdrawAll", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): WithdrawAll | null {
    return changetype<WithdrawAll | null>(
      store.get_in_block("WithdrawAll", id.toHexString()),
    );
  }

  static load(id: Bytes): WithdrawAll | null {
    return changetype<WithdrawAll | null>(
      store.get("WithdrawAll", id.toHexString()),
    );
  }
